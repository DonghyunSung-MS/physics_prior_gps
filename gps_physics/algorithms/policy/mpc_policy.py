...
class MPCPolicy(Policy):
    ...
    def forward(self, prev_mean_traj, inital_state, AB, c, cost):
        ...
        P = sparse.vstack(
            [sparse.hstack([sparse.block_diag(l_xxs), l_xu_block]), sparse.hstack([l_ux_block, sparse.block_diag(l_uus)])],
            format="csc",
        )
        ...
        # constraint
        # linearized dynamics
        Ax = sparse.kron(sparse.eye(T + 1), -sparse.eye(x_dim)) + sparse.hstack(
            [
                sparse.vstack([sparse.csc_matrix((x_dim, T * x_dim)), sparse.block_diag(A)]),
